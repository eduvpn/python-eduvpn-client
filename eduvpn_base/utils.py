...
def get_target_prefix(target: str, local: str) -> Optional[str]:
    """
    Returns the Python prefix where the base eduVPN package is installed
...
def get_base_prefix() -> str:
    local = f"{path.dirname(path.abspath(__file__))}/data"
    prefix = get_target_prefix("share/eduvpn-base/builder/mainwindow.ui", local)
    if not prefix:
        raise Exception("Can't find eduVPN base installation")
    return prefix


@lru_cache(maxsize=1)
def get_variant_prefix(variant_pretty_name, variant_name) -> str:
    local = f"{path.dirname(path.abspath(__file__))}/../{variant_name}/{variant_name}/data"
    prefix = get_target_prefix(f"share/{variant_name}/images/logo.png", local)
    if not prefix:
        raise Exception(f"Can't find {variant_pretty_name} installation")
    return prefix
