...
def switched(meta, builder, verifier, lets_connect):
    ...
    if not state:
        ...
        notify(notification, u"eduVPN disconnecting...", u"Disconnecting from {}".format(meta.display_name))
        ...
        try:
            ...
            error_helper(window, u"can't disconnect", "{}: {}".format(type(e).__name__, str(e)))
            GLib.idle_add(lambda: switch.set_active(True))
            raise
