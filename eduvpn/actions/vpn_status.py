...
def vpn_change(builder, selected):
        ...
        try:
            if active.Uuid == selected['uuid']:
                ...
                if active.State == 2:  # activated
                    ...
                    logger.info("setting switch ON")
                    switch.set_active(True)
                    notify("eduVPN connecting...", "Activating '{}'".format(selected['display_name']))
                else:
    ...
    if not selected_uuid_active:
        logger.info("Our selected profile not active {}".format(selected['uuid']))
